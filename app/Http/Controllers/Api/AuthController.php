...
class AuthController extends Controller
{
    ...
    public function splashScreens(){
        $splash_screens = SplashScreen::select('type', 'heading', 'content', 'image')
        ->get()
        ->map(function ($screen) {
            $screen->image = $screen->image ? asset($screen->image) : null;
            return $screen;
        });

        if ($splash_screens->isEmpty()) {
            return ApiResponse::successResponsenoData('No Data Found');
        }

        $data = $splash_screens->groupBy('type')->map(function ($screens) {
        ...
        return ApiResponse::successResponse($data, 'Splash Screen Data Found');
